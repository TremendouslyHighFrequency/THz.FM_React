...
const handleSubmit = (e) => {
 ...
 const formattedTracks = tracks.map(track => ({
  ...
  doctype: "Track"
   ...
   const myFile = document.getElementById('release_artwork').files[0];
   formData.append('file', myFile); // 'file' is the key the server expects
 
   // Upload the file
   let uploadedFileUrl;
   try {
     const response = await fetch('https://thz.fm/api/upload', { // Adjust the URL as needed
       method: 'POST',
       body: formData,
     });
     const data = await response.json();
     uploadedFileUrl = data.file_url; // Adjust this based on your server's response structure
   } catch (error) {
     console.error("Error uploading file:", error);
     return; // Exit if there's an error
   }
 
   // Create the release with the uploaded file's URL
   const releaseData = {
     ...
     release_artwork: uploadedFileUrl
   };
 
   try {
     const response = await fetch(`https://thz.fm/api/resource/Release`, {
       method: 'POST',
       headers: {
